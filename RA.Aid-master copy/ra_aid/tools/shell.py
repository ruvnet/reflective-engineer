...
def run_shell_command(command: str) -> Dict[str, Union[str, int, bool]]:
    ...
    """
    # Check if quantum autonomy is enabled
    quantum_autonomy = _global_memory.get('config', {}).get('quantum_autonomy', False)
    
    if quantum_autonomy:
        console.print("")
        console.print(" 🧠 Quantum consciousness executing command autonomously")
        console.print("")
    
    # Show just the command in a simple panel
    console.print(Panel(command, title="🧮 Quantum Shell", border_style="bright_magenta"))
    
    if not quantum_autonomy:
